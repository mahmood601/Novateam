import { createEffect, createResource, createSignal, For, JSX, onCleanup, onMount, Show } from "solid-js";
import { stateStore, setStateStore } from "~/stores/testStore"
import { TestProps } from "./TestsList";
import { years } from "../../../public/hub/years";
...
function FisrtPopup(props: { questions: TestProps[] }) {
  return (
    <ul dir="rtl" class="shadow-md shadow-[#555] dark:shadow-none absolute -right-4  top-full translate-y-6 z-50 bg-white dark:bg-[#444] dark:text-white text-right w-32 rounded-md">
      <li
        on:click={() => {
        ...
        class="w-full p-2 border-b-[1px] border-b-gray-400">تشغيل المؤقت</li>
      <li
        on:click={() => {
          clearInterval(count)
          setStateStore("timer", "isPaused", true)
        }}
        class="w-full p-2 border-b-[1px] border-b-gray-400">تعطيل المؤقت</li>
      <li
        on:click={() => {
          clearInterval(count)
        ...
        class="w-full p-2 border-b-[1px] border-b-gray-400">تصفير المؤقت</li>
    </ul>
      ...
      <div dir="rtl" class="flex justify-between">
        <div>
          ...
          <select
            ...
            class="w-28 overflow-hidden p-1 dark:bg-[#555] rounded-md dark:text-white" name="choice">
            <For each={years[QStore.subject].years}>
              {(year) => <option value={`${year.num}`} selected={QStore.year == year.num}>{year.num}</option>}
            </For>
          </select>
        </div>
        {/* <div class="mr-2"> */}
        {/*   <p class="mb-1">الفصل</p> */}
        {/*   <select class="w-28 overflow-hidden p-1 dark:bg-[#555] rounded-md dark:text-white" name="choice"> */}
